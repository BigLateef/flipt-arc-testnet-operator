...
EXIT_CHECK_ON_TICK = os.getenv('EXIT_CHECK_ON_TICK', '0') == '1'
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == '/status':
            try:
                current = status()
                current.update({'executor_endpoint': '/execute', 'auto_buy_loop': AUTO_BUY_LOOP, 'exit_check_on_tick': EXIT_CHECK_ON_TICK, 'auto_min_fdv_usd': os.getenv('AUTO_MIN_FDV_USD', '2000'), 'max_auto_buys': os.getenv('MAX_AUTO_BUYS', '3'), 'target_configured': bool(os.getenv('TARGET_LAUNCH_ADDRESS', '').strip()), 'burner_configured': bool(os.getenv('BURNER_PRIVATE_KEY', '').strip())})
                self.send_json(current)
            except Exception as exc: self.send_json({'ok': False, 'error': str(exc)}, 503)
            return
        if parsed.path == '/tick':
            if not self.secret_ok(query): self.send_json({'ok': False, 'error': 'unauthorized'}, 401); return
            try:
                ...
                if AUTO_BUY_LOOP and LIVE_TRADING == '1' and DRY_RUN != '1':
                    ...
                    if EXIT_CHECK_ON_TICK:
                        from exit_monitor import check
                        result['exit_monitor'] = check()
                    else:
                        result['exit_monitor'] = {'status': 'deferred', 'reason': 'read-only exit check disabled on buy tick'}
